...
export function LeadsContent({ userId }: { userId: string | null }) {
  ...
  const searchLeads = async (
    ...
    setIsSearching(true);
    try {
      let url = `https://dev-api.findsocial.io/leads/leads/search?auth0_id=${userId}&search_term=${encodeURIComponent(
        searchTerm.trim()
      )}&page=${page}&limit=${limit}&sort_by=timestamp&sort_order=desc`;
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      const url = `https://dev-api.findsocial.io/leads/leads/${
        leadToDelete.id
      }?auth0_id=${userId}&platform=${encodeURIComponent(
        leadToDelete.platform
      )}`;
  ...
  const handleExportToCSV = async () => {
    ...
    setIsExporting(true);
    try {
      let url = `https://dev-api.findsocial.io/leads/download/csv?auth0_id=${userId}`;

      // Add platform filter if any platforms are applied
      if (appliedPlatforms.length > 0) {
        const platformParam = appliedPlatforms.join(",");
        url += `&platform=${encodeURIComponent(platformParam)}`;
      }

      // Add search term if searching
      if (searchQuery.trim()) {
        url += `&search_term=${encodeURIComponent(searchQuery.trim())}`;
      }
  ...
  const handleExportToPDF = async () => {
    ...
    try {
      let url = `https://dev-api.findsocial.io/leads/download/pdf?auth0_id=${userId}`;
      ...
      if (link.download !== undefined) {
        ...
        link.setAttribute(
          "download",
          `leads_export_${new Date().toISOString().split("T")[0]}.pdf`
        );
        link.style.visibility = "hidden";
        document.body.appendChild(link);
        link.click();
    ...
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="relative">
              <Button
                ...
                className={`cursor-pointer border-[#d0d5dd] text-[#344054] hover:bg-white hover:ring-[#7f56d9] hover:text-[#7f56d9] hover:ring-2 h-10 px-4 flex items-center gap-2 bg-transparent relative ${
                  ...
                  activeFilterCount > 0
                    ? "ring-2 ring-[#7f56d9] text-[#7f56d9]"
                    : ""
                }`}
              >
          ...
          {/* Action Buttons Row - Separate row on mobile */}
        ...
        {searchQuery && (
          <div className="flex items-center gap-2 text-sm text-[#667085] bg-[#f9fafb] px-4 py-2 rounded-lg border border-[#eaecf0]">
            ...
            <span>
              Search results for "
              <span className="font-medium text-[#344054]">{searchQuery}</span>"
              {totalRecords > 0 && (
                <span className="ml-1">({totalRecords} results)</span>
              )}
            </span>
            {isSearching && (
        ...
        <div className="bg-white rounded-lg border border-[#eaecf0] overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <div className="hidden md:block border-b border-[#eaecf0] bg-[#f9fafb] px-6 py-3">
              <div
                className="flex items-center gap-4 text-xs font-medium text-[#667085] uppercase tracking-wide w-full"
              ...
              >
                ...
                {columnSettings.country && (
                  <div className="flex-[1.5] min-w-[128px] flex items-center gap-1">
                    Country
                  </div>
                )}
                    ...
                    <div className="space-y-2">
                      <div>
                        ...
                        <div className="text-sm text-[#475467] truncate">
                          {lead.data.email &&
                          Array.isArray(lead.data.email) &&
                          lead.data.email.length > 0
                  ...
                  >
                    ...
                    {columnSettings.followers && (
                      <div className="flex-1 min-w-[96px] flex items-center">
                        <span className="text-sm text-[#475467] truncate">
                          {lead.data.followers
                            ? lead.data.followers.toLocaleString()
                            : "N/A"}
                        </span>
                      </div>
                    )}
                    ...
                    {columnSettings.following && (
                      <div className="flex-1 min-w-[96px] flex items-center">
                        <span className="text-sm text-[#475467] truncate">
                          {lead.data.following
                            ? lead.data.following.toLocaleString()
                            : "N/A"}
                        </span>
                      </div>
                    )}
                    ...
                    {columnSettings.posts && (
                      <div className="flex-1 min-w-[80px] flex items-center">
                        <span className="text-sm text-[#475467] truncate">
                          {lead.data.posts || lead.data.post
                            ? (
                                lead.data.posts || lead.data.post
                              )?.toLocaleString()
                            : "N/A"}
              ...
              <div className="p-6">
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div className="border-t border-[#eaecf0] pt-4">
                    <p className="text-xs font-medium text-[#667085] mb-3 uppercase tracking-wide">
                      Additional Columns
                    </p>
              ...
              <div className="p-6">
                  ...
                  <div className="w-10 h-10 bg-red-100 rounded-full flex items-center justify-center">
                    ...
                    >
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.732-.833-2.502 0L4.35 15.5c-.77.833.192 2.5 1.732 2.5z"
                      />
                    </svg>
                  </div>
                  <div>
                ...
                <div className="mb-6">
                  <p className="text-sm text-[#475467]">
                    ...
                    from{" "}
                    <span className="font-medium text-[#101828] capitalize">
                      {leadToDelete.platform}
                    </span>
                    ?
                  </p>
