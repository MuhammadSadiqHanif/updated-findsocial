...
export function AddRecordsModal({ open, onOpenChange, onAddRecords }: AddRecordsModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[1400px] w-[95vw] max-h-[85vh] p-0 bg-white overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
            <div className="bg-white rounded-lg border-t border-[#eaecf0]">
              <div className="border-b border-[#eaecf0] bg-[#f9fafb] px-6 py-3 sticky top-0 z-10 hidden md:grid grid-cols-[40px_1fr_1fr_200px_200px] gap-6 items-center text-xs font-medium text-[#667085] uppercase tracking-wide">
                  <Checkbox
                  ...
                  <div className="flex items-center gap-1">Email address</div>
                  <div className="flex items-center gap-1">Country</div>
                  <div>URL/Links</div>
              </div>
              ...
              <div className="divide-y divide-[#eaecf0]">
                {availableRecords.map((record) => (
                  <div key={record.id} className="px-6 py-4 hover:bg-[#f9fafb] transition-colors">
                    <div className="grid grid-cols-1 md:grid-cols-[40px_1fr_1fr_200px_200px] gap-y-3 md:gap-6 items-center">
                      <div className="flex justify-between items-center md:block">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="md:hidden">
                                <div className="text-sm font-medium text-[#101828]">{record.name}</div>
                                <div className="text-sm text-[#667085]">{record.username}</div>
                            </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Button variant="ghost" size="sm" className="p-1.5 h-8 w-8 text-[#667085] hover:text-[#344054] hover:bg-[#f2f4f7] rounded-md flex-shrink-0"><LinkIcon className="w-4 h-4" /></Button>
                            )}
                            {record.socialLinks.instagram && (
                            <Button variant="ghost" size="sm" className="p-1.5 h-8 w-8 text-[#667085] hover:text-[#344054] hover:bg-[#f2f4f7] rounded-md flex-shrink-0"><Instagram className="w-4 h-4" /></Button>
                            )}
                            {record.socialLinks.spotify && (
                            <Button variant="ghost" size="sm" className="p-1.5 h-8 w-8 text-[#667085] hover:text-[#344054] hover:bg-[#f2f4f7] rounded-md flex-shrink-0"><Spotify className="w-4 h-4" /></Button>
                            )}
                        </div>
                      </div>
                    </div>
        ...
        <div className="px-6 py-4 flex-shrink-0 border-t border-[#eaecf0]">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-2 text-sm text-[#344054]">
                ...
                <span className="hidden sm:inline">Showing per page</span>
                ...
                <div className="hidden sm:flex items-center gap-1">
                  {[1, 2, 3].map((page) => (
                    <Button
                      ...
                      size="sm"
                      className={ currentPage === page ? "border-[#d0d5dd] bg-[#f9fafb] text-[#344054] w-8 h-8 p-0" : "text-[#667085] hover:bg-[#f9fafb] w-8 h-8 p-0"}
